...
//void main() => runApp(IAPSample());

class IAPSample extends StatefulWidget {
  @override
  _IAPSampleState createState() => _IAPSampleState();
}

class _IAPSampleState extends State<IAPSample> {
  final List<String> _productLists = Platform.isAndroid
      ...
      : ['com.instantonnection.app.develop.point1000'];
...
//    try {
//      bool success = await billing.subscribe(item.productId);
//      print(success);
//    } catch (error) {
//      print(error);
//    }
  ...
  Future<Null> _getSubscriptions() async {
    List<IAPItem> items =
        await FlutterInappPurchase.getSubscriptions(_subscriptionLists);
    for (var item in items) {
      print('${item.toString()}');
      this._items.add(item);
    }
...
//    List<BillingProduct> subscriptions =
//        await billing.getSubscriptions(_subscriptionLists);
//
//    List<IAPItem> items = subscriptions.map((billing) {
//      Map<String, dynamic> json = <String, dynamic>{
//        "productId": billing.identifier,
//        'price': billing.price,
//        'title': billing.title,
